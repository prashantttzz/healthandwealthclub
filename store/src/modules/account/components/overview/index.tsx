  ...
  return (
    <div data-testid="overview-page-wrapper" className="flex flex-col gap-8 lg:gap-12  animate-in fade-in slide-in-from-bottom-4 duration-700 md:mt-10">
      <div className="flex flex-col gap-3">
        ...
        <p className="font-manrope text-[14px] text-accent/50 max-w-lg leading-relaxed px-6 lg:px-0">
      ...
      <div className="flex lg:hidden flex-col gap-0 border-t border-accent/5">
        <MobileMenuTile 
          ...
          label="Dossier & Orders" 
          desc="Check your order status and history" 
        />
        <MobileMenuTile 
          href="/account/addresses" 
          icon={<HugeiconsIcon icon={Location01Icon} size={22} />} 
          label="Address Book" 
          desc="Manage shipping for hassle-free checkout" 
        />
        <MobileMenuTile 
...
const MobileMenuTile = ({ href, icon, label, desc }: { href: string, icon: React.ReactNode, label: string, desc: string }) => (
  <LocalizedClientLink href={href} className="group">
    <div className="flex items-center justify-between p-6 bg-bg border-b border-accent/5 group-active:bg-accent/[0.02] transition-colors">
      <div className="flex items-center gap-6">
        <div className="text-accent/30 group-hover:text-accent transition-colors">
          {icon}
        </div>
        <div className="flex flex-col gap-0.5">
          <span className="font-manrope text-[14px] font-bold text-accent">{label}</span>
          <span className="font-manrope text-[11px] text-accent/40">{desc}</span>
        </div>
      </div>
      <HugeiconsIcon icon={ArrowRight01Icon} size={18} className="text-accent/10 group-hover:text-accent transition-all" />
    </div>
  </LocalizedClientLink>
)
