...
import { Check, X, Minus, Plus, ChevronRight, ChevronLeft, Home, Truck, CreditCard, ShieldCheck, Tag, MoreHorizontal, Banknote, ShoppingBag, ArrowRight, Smartphone } from "lucide-react"
          ...
          >
            ...
            `}>
              {done ? (
                ...
                <span className={`font-manrope text-[11px] font-bold transition-colors duration-500 ${active ? "text-accent" : "text-accent/80"}`}>
                  {idx + 1}
                </span>
              )}
            </div>
  ...
  return (
    <div className={`flex flex-col gap-10 transition-opacity duration-300 ${updatingId === "promo" ? "opacity-30 pointer-events-none" : ""}`}>
            ...
            <div className="flex-1 flex flex-col justify-between min-w-0">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-manrope text-lg md:text-xl text-accent font-regular leading-snug truncate">{item.product_title}</p>
              ...
              <div className="flex items-center gap-4 mt-4">
                ...
                <div className="flex items-center border border-accent/10">
                  <button onClick={() => changeQty(item.id, item.quantity - 1)} className="w-7 h-7 flex items-center justify-center text-accent/30 hover:text-accent hover:bg-accent/5 transition-all">{Ico.minus("w-3.5 h-3.5")}</button>
                  ...
                  <button onClick={() => changeQty(item.id, item.quantity + 1)} className="w-7 h-7 flex items-center justify-center text-accent/30 hover:text-accent hover:bg-accent/5 transition-all">{Ico.plus("w-3.5 h-3.5")}</button>
                </div>
              </div>
              <div className="mt-4">
      ...
      <div className="flex items-center justify-between border border-accent bg-accent text-bg p-6">
        ...
        <button onClick={() => setCouponOpen(true)} className="font-manrope text-[12px] uppercase font-semibold tracking-[0.15em] text-bg flex items-center gap-1  transition-colors">Apply Coupon {Ico.chevRight("w-4 h-4")}</button>
      </div>
...
const OrderSummary = ({ currentStep, onContinue, selectedAddress }: {
  currentStep: number; onContinue: () => void; selectedAddress: HttpTypes.StoreCustomerAddress | null
}) => {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="border-t border-dashed border-bg/10 pt-6 flex justify-between items-center">
        ...
        <span className="font-manrope text-xl text-bg font-semibold">{convertToLocale({ amount: (subtotal || 0) - (cart.discount_total || 0), currency_code: cart.currency_code })}</span>
      </div>
