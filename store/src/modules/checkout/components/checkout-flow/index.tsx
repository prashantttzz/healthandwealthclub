      ...
      <div className="fixed inset-0 bg-black/30 z-[200] flex items-center justify-center p-6">
        <div className="bg-bg p-12 max-w-md w-full flex flex-col items-center gap-8 shadow-2xl animate-in zoom-in-95 fade-in duration-500">
          ...
          <div className="bg-accent/[0.03] border border-accent/5 p-6 w-full">
            <div className="flex justify-between font-manrope text-[13px] uppercase tracking-widest font-bold"><span className="text-accent/40">Total Paid</span><span className="text-accent">{convertToLocale({ amount: (optimisticItems.reduce((acc, i) => acc + (i.unit_price || 0) * i.quantity, 0)) - (cart.discount_total || 0), currency_code: cart.currency_code })}</span></div>
          </div>
    ...
    <div className="min-h-screen bg-bg">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-bg border-t shadow-[0_-4px_20px_rgba(0,0,0,0.03)] border-accent/5 px-4 py-3 z-50 flex items-center justify-between">
        <div onClick={() => setDrawerOpen(true)} className="cursor-pointer">
          <p className="font-newsreader italic text-xl text-accent">{convertToLocale({ amount: (useMemo(() => optimisticItems.reduce((acc, i) => acc + (i.unit_price || 0) * i.quantity, 0), [optimisticItems])) - (cart.discount_total || 0), currency_code: cart.currency_code })}</p>
